#!/usr/bin/env python3
"""
Gera relatório DETALHADO de validação NSGA-III com visualização passo a passo
"""
...
class DetailedValidatorNSGA3:
    ...
    def parse_nsga3_solution(self):
        """
        Parse solution file in NSGA3 format
        Format:
        === ROTA DO VEÍCULO X ===
        Depósito (0) [X: ..., Y: ...]
        Cliente N [X: ..., Y: ...]
        ...
        with open(self.solution_file, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                if '=== ROTA DO VEÍCULO' in line:
                    if current_route and any(c != 0 for c in current_route):
                        routes.append(current_route)
                    current_route = []
                    in_route = True
                    continue

                if in_route and line:
                    # Parse cliente: "Cliente 5 [X: 42,0, Y: 65,0]"
                    # ou depósito: "Depósito (0) [X: 40,0, Y: 50,0]"
                    if line.startswith('Cliente'):
                        match = re.search(r'Cliente (\d+)', line)
                        if match:
                            customer_id = int(match.group(1))
                            current_route.append(customer_id)
                    elif line.startswith('Depósito') and current_route:
                        # Fim da rota
                        if any(c != 0 for c in current_route):
                            routes.append(current_route)
                        current_route = []
                        in_route = False

        # Adiciona última rota se existir
        if current_route and any(c != 0 for c in current_route):
            routes.append(current_route)
    ...
    def load_solution(self):
        ...
        self.routes = self.parse_nsga3_solution()
    ...
    def generate_detailed_report(self):
        ...
        # Header
        self.write_line("="*80)
        self.write_line("RELATÓRIO DETALHADO DE VALIDAÇÃO NSGA-III")
        self.write_line("="*80)
        self.write_line()
        self.write_line(
...
def main():
    if len(sys.argv) < 4:
        print("Uso: python3 generate_detailed_validation_nsga3.py <instance_file> <solution_file> <output_file>")
        print()
        print("Exemplo:")
        print("  python3 generate_detailed_validation_nsga3.py app/src/main/java/instances/solomon/C101.txt \\")
        print("          resultsNSGA3/evo_c101_exec01.txt \\")
        print("          results_validation_NSGA3_C1_detailed/C101/evo_c101_exec01.txt")
        sys.exit(1)
    ...
    validator = DetailedValidatorNSGA3(instance, solution_file, output_file)
